...
def run_model_and_save_samples(args, pipeline, caption_refiner_model=None, enhance_video_model=None):
    ...
    def generate_long_videos(prompt, conditional_pixel_values_path=None, mask_type=None, prompt_index=0, num_chunks=5):
        ...
        for i_chunk in range(num_chunks):
            ...
            if i_chunk == 0:
                ...
                if type(conditional_pixel_values_path) == str and conditional_pixel_values_path.endswith('.mp4'):
                    vr = VideoReader(conditional_pixel_values_path)
                    video_length = len(vr)
                    batch_index = [0] # last frame
                    cond_frame = vr.get_batch(batch_index).asnumpy()[0] # h w c
                    cond_image = Image.fromarray(cond_frame) # this is for kstep evaluation
                    conditional_pixel_values_path = None
                    conditional_pixel_values_input = [cond_image] # h w c
                else:
                    cond_image = Image.open(conditional_pixel_values_path[0]) # opsr1.3 preprocess to [path]
            ...
            init_noises = [torch.randn((1, 8, 24, 60, 80)) for _ in range(10)]
            k_step = 5
    ...
    if args.model_type == "iterative":
        ...
        text_path = "/home/szhang3/siyang-storage/opsrplan1.3/examples/all_prompts_cherry.txt"
        text_prompts = open(text_path, 'r').readlines()
        text_prompts = [i.strip() for i in text_prompts]

        id2prompt = dict()
        id2cond_path = dict()

        for line in text_prompts:
            entries = line.split(" ")
            prompt_id = int(entries[0])
            prompt = " ".join(entries[1:])
            id2prompt[prompt_id] = prompt
            cond_path = f"/home/szhang3/siyang-storage/opsrplan1.3/sample_t2v_results2/EulerAncestralDiscrete_{prompt_id}_gs7.5_s30.mp4"
            id2cond_path[prompt_id] = cond_path

        
        test_indices = [2, 3, 4, 10, 19, 24]
        ...
        for index in test_indices:
            ...
            prompt = id2prompt[prompt_index]
            # cond_path = conditional_pixel_values_path[prompt_index]
            cond_path = id2cond_path[prompt_index]

            if prompt_index not in test_indices:
                continue

            print(prompt_index, prompt, cond_path)
            generate_long_videos(prompt, cond_path, mask_type, num_chunks=5, prompt_index=prompt_index)
...
def get_args_i2v():
    ...
    parser.add_argument("--save_img_path", type=str, default="/home/szhang3/siyang-storage/opsrplan1.3/sample_i2v_results_supp_base")
