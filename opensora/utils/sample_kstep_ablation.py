...
def run_model_and_save_samples(args, pipeline, caption_refiner_model=None, enhance_video_model=None):
    ...
    def generate_long_videos(prompt, conditional_pixel_values_path=None, mask_type=None, prompt_index=0, num_chunks=5):
        ...
        for j in range(2):
            ...
            noise_dir = os.path.join(prompt_dir, f'noise_{j}')
            os.makedirs(noise_dir, exist_ok=True)
            torch.save(noise, os.path.join(noise_dir, 'noise.pt'))
            ...
            for step in [3, 4, 5, 6, 7, 8, 10, 20, 25, 50]:
                ...
                torchvision.io.write_video(os.path.join(noise_dir, f'step_{step}.mp4'), videos, args.fps)
...
def get_args_i2v():
    ...
    parser.add_argument("--text_encoder_name_2", type=str, default=None)
    parser.add_argument("--save_img_path", type=str, default="/home/szhang3/siyang-storage/opsrplan1.3/kstep_ablation")
    parser.add_argument("--guidance_scale", type=float, default=7.5)
