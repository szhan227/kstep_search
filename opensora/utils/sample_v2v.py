print("Start to call sample_v2v.py")
    ...
    def generate_long_videos(prompt, conditional_pixel_values_path=None, mask_type=None, prompt_index=0, num_chunks=5):
        ...
        for i_chunk in range(num_chunks):
            ...
            if i_chunk == 0:
                ...
                # video_chunks = [t h w c, ...]
                # conditional_pixel_values_input = [video_chunks[-1][-1]] # list of h w c, torch.uint8 tensor
                conditional_pixel_values_input = long_video[-46:].detach().clone() # [46, 480, 640, 3]
                conditional_pixel_values_input = [ff for ff in conditional_pixel_values_input] # list of h w c, torch.uint8 tensor
            ...
            if long_video is None:
                video_new = videos.detach().clone()
                long_video = videos.detach().clone()
            else:
                video_new = videos.detach().clone()[-47:]
                long_video = torch.cat([long_video, video_new], dim=0)

            torchvision.io.write_video(os.path.join(chunk_dir, f'chunk_{i_chunk}.mp4'), video_new, args.fps)
...
def get_args_i2v():
    ...
    parser.add_argument("--save_img_path", type=str, default="/home/szhang3/siyang-storage/opsrplan1.3/sample_v2v_results")
