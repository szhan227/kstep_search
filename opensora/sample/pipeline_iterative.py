...
def get_pixel_values(file_path, num_frames, conditional_pixel_values_input=None):
    # conditional_pixel_values_input should be a list of PIL Image
    if conditional_pixel_values_input is not None:
        pixel_values = [torch.from_numpy(np.array(image)) for image in conditional_pixel_values_input]
        pixel_values = [rearrange(image, 'h w c -> c h w').unsqueeze(0) for image in pixel_values]
    elif is_image_file(file_path[0]):
...
class OpenSoraIterativePipeline(OpenSoraPipeline):
    ...
    def check_inputs(
        ...
        if conditional_pixel_values_input is None:
    ...
    def get_mask_type_cond_indices(self, mask_type, conditional_pixel_values_path, conditional_pixel_values_indices, num_frames,
        ...
        if conditional_pixel_values_input is not None:
            num_guide_frames = len(conditional_pixel_values_input)
            mask_type = MaskType.i2v if num_guide_frames == 1 else MaskType.continuation
            conditional_pixel_value_indices = list(range(num_guide_frames))
            return mask_type, conditional_pixel_value_indices
    ...
    def __call__(
        ...
        seed_for_step = None,
        prev_chunk_model_output_every_step = None,
        gamma_schedule = None,
    ):
        ...
        conditional_pixel_values = get_pixel_values(conditional_pixel_values_path, num_frames, conditional_pixel_values_input=conditional_pixel_values_input)
        ...
        num_progress_bar_steps = num_inference_steps if early_stop_step is None else min(early_stop_step, num_inference_steps)

        if seed_for_step is not None:
            generator_for_step = torch.Generator(device=device)
            generator_for_step.manual_seed(seed_for_step) # fix the same noise added to each step function
            extra_step_kwargs['generator'] = generator_for_step

        noise_every_step_dict = dict()
        with self.progress_bar(total=num_progress_bar_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if early_stop_step is not None and i >= early_stop_step:
                    print("Early stopping at step", i, "out of", num_inference_steps)
                    break
                if self.interrupt:
                    continue
                ...
                t_idx = t.long().item()
                # EMA
                if prev_chunk_model_output_every_step is not None and gamma_schedule is not None:
                    last_chunk_model_output_t = prev_chunk_model_output_every_step[t_idx]
                    noise_pred = (1 - gamma_schedule) * last_chunk_model_output_t + gamma_schedule * noise_pred
                
                
                noise_every_step_dict[t_idx] = noise_pred
        ...
        return dict(videos=videos, noise_every_step_dict=noise_every_step_dict)